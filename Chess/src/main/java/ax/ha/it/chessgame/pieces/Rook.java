...
public class Rook extends Piece {
        ...
        try {
            ...
            if (color.equals("Black")) {
                this.image = SwingFXUtils.toFXImage(bi.getSubimage(
                            2 * 64, 0 * 64, 64, 64), null);
            } else {
                this.image = SwingFXUtils.toFXImage(bi.getSubimage(
                            2 * 64, 1 * 64, 64, 64), null);
            }
        } catch (Exception e) {
    ...
    public List<Square> getMoves(Board board, int x, int y) {
        ...
        moveInLine(list, possibleMoves, x, y, 1, 0);
        moveInLine(list, possibleMoves, x, y, 0, 1);
        moveInLine(list, possibleMoves, x, y, -1, 0);
        moveInLine(list, possibleMoves, x, y, 0, -1);
    ...
    public void moveInLine(Square[][] list , List<Square> possibleMoves, int x, int y, int xDirection, int yDirection) {
